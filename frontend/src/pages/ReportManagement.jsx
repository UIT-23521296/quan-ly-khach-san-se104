...
const ReportManagement = () => {
  ...
  const renderTimeText = (rptMonth, rptYear) => {
      // Nếu là báo cáo năm (rptMonth là null, undefined hoặc 'ALL')
      ...
      // Nếu xem tháng hiện tại của năm hiện tại -> Tính đến ngày hôm nay
      if (m === currentMonth && y === currentYear) {
          const currentDay = today.getDate();
          return `(Số liệu tính đến ngày: ${fmt(currentDay)}/${fmt(m)}/${y})`;
      } 
      
      // Nếu xem tháng cũ/tương lai -> Tính full tháng
        ...
        <html>
          ...
          <body>
            ...
            <table>
              ...
              <tbody>
                ${data.map((item, index) => {
                    const percent = total > 0 ? ((item.DoanhThu / total) * 100).toFixed(2) : 0;
                    return `
                      <tr>
                        <td>${index + 1}</td>
                        <td class="text-left">${item.TenLoaiPhong || item.MaLoaiPhong}</td>
                        <td class="text-right">${Number(item.DoanhThu).toLocaleString('vi-VN')} đ</td>
                        <td>${percent}%</td>
                      </tr>
                    `;
                }).join('')}
                <tr class="total-row">
                  ...
                  <td class="text-right">${total.toLocaleString('vi-VN')} đ</td>
                  <td>100%</td>
                </tr>
              </tbody>
            </table>
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      setLiveTotal(res.data.reduce((sum, item) => sum + Number(item.DoanhThu), 0));
  ...
  const handleSave = async () => {
    ...
    try {
        await api.post("/baocao/save", { thang: reportType === 'month' ? month : 'ALL', nam: year });
  ...
  // --- API HANDLERS (TAB 2: SAVED HISTORY) ---
  const fetchSavedReports = async () => {
      try {
          // Logic lọc: Nếu filterType là 'year' thì gửi type=year để backend chỉ lấy báo cáo năm
          let url = `/baocao/list?nam=${filterSavedYear}`;
          
          if (filterType === 'year') {
              url += `&type=year`;
          } else {
              url += `&type=month`;
              if (filterSavedMonth !== 'ALL') {
                  url += `&thang=${filterSavedMonth}`;
              }
          }
  ...
  }, [activeTab, filterType, filterSavedMonth, filterSavedYear]);
  ...
  const handleViewDetail = async (report) => {
      ...
      try {
          ...
          setDetailTotal(res.data.reduce((sum, item) => sum + Number(item.DoanhThu), 0));
  ...
  const handleDelete = async (id) => {
      ...
      try {
          ...
          alert("✅ Đã xóa!");
          fetchSavedReports();
          // Nếu đang xem báo cáo bị xóa thì clear màn hình chi tiết
          if (selectedReport?.MaBaoCao === id) { setSelectedReport(null); setDetailData(null); }
        ...
        <table style={styles.table}>
            ...
            <tbody>
                {data.map((item, index) => {
                    ...
                    return (
                        <tr key={index} style={{borderBottom: '1px solid #f1f5f9'}}>
                            ...
                            <td style={{...styles.td, fontWeight: '600'}}>{item.TenLoaiPhong || item.MaLoaiPhong}</td>
                ...
                <tr style={{background: '#f8fafc', borderTop: '2px solid #334155'}}>
                    <td colSpan={2} style={{padding: '15px', fontWeight: 'bold'}}>TỔNG CỘNG</td>
  ...
  return (
    <div style={styles.wrapper}>
      ...
      <div style={styles.tabContainer}>
          <button style={activeTab === 'create' ? styles.tabActive : styles.tab} onClick={() => setActiveTab('create')}>📝 Lập báo cáo mới</button>
          <button style={activeTab === 'saved' ? styles.tabActive : styles.tab} onClick={() => setActiveTab('saved')}>📂 Danh sách đã lưu</button>
      </div>
          ...
          <div style={{animation: 'fadeIn 0.3s'}}>
              <div style={styles.filterCard}>
                  <div style={{display: 'flex', gap: '20px', marginBottom: '15px'}}>
                      <label style={styles.radioLabel}><input type="radio" checked={reportType === 'month'} onChange={() => setReportType('month')}/> Theo Tháng</label>
                      <label style={styles.radioLabel}><input type="radio" checked={reportType === 'year'} onChange={() => setReportType('year')}/> Theo Năm</label>
                  </div>
                  <div style={styles.filterRow}>
                      ...
                      <input type="number" value={year} onChange={(e) => setYear(e.target.value)} style={styles.input} placeholder="Năm"/>
                      <button onClick={handlePreview} style={styles.btnPrimary} disabled={loading}>{loading ? "..." : "Xem trước"}</button>
                  </div>
              </div>
                  ...
                  <div>
                      <div style={styles.resultHeader}>
                          ...
                          <div style={{display: 'flex', gap: '10px'}}>
                              <button 
                                onClick={() => handlePrint(`Báo cáo doanh thu (Tạm tính)`, renderTimeText(reportType === 'month' ? month : 'ALL', year), liveData, liveTotal)} 
                                style={styles.btnPrint}
                              >
                                  🖨️ In Báo Cáo
                              </button>
                              <button onClick={handleSave} style={styles.btnSuccess}>💾 Lưu vào hệ thống</button>
                          </div>
                      </div>
      ...
      {activeTab === 'saved' && (
          <div style={{display: 'flex', gap: '20px', animation: 'fadeIn 0.3s'}}>
              ...
              <div style={{width: '350px', background: 'white', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', maxHeight: '80vh', display: 'flex', flexDirection: 'column'}}>
                  <div style={{padding: '15px', borderBottom: '1px solid #e2e8f0'}}>
                      <div style={{fontWeight: 'bold', color: '#334155', marginBottom: '10px'}}>Lịch sử báo cáo</div>
                      ...
                      <div style={{display: 'flex', gap: '8px'}}>
                          ...
                          <input 
                            ...
                            onChange={(e) => setFilterSavedYear(e.target.value)}
                            placeholder="Năm"
                            style={{...styles.inputSmall, flex: filterType === 'year' ? 1 : 'unset', width: filterType === 'year' ? '100%' : '80px'}}
                          />
                      </div>
                  </div>
                                  ...
                                  <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                                      <div style={{display: 'flex', flexDirection: 'column'}}>
                                          ...
                                          <span style={{fontSize: '11px', color: '#94a3b8', fontStyle: 'italic', marginTop: '2px'}}>
                                              ⏰ Lập lúc: {new Date(rpt.NgayTao).toLocaleTimeString('vi-VN', {hour: '2-digit', minute:'2-digit', second:'2-digit'})}
                                          </span>
                                      </div>

                                      <span 
                                        onClick={(e) => { e.stopPropagation(); handleDelete(rpt.MaBaoCao); }}
              ...
              <div style={{flex: 1}}>
                  {selectedReport && detailData ? (
                      <div style={{background: 'white', padding: '20px', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)'}}>
                          <div style={styles.resultHeader}>
                              <div>
                                  <h2 style={{margin: '0 0 5px 0', color: '#1e293b', fontSize: '1.5rem'}}>{selectedReport.TenBaoCao}</h2>
                                  <p style={styles.timeText}>
                                      {renderTimeText(selectedReport.Thang, selectedReport.Nam)}
                                  </p>
                              </div>
                              <button 
                                onClick={() => handlePrint(selectedReport.TenBaoCao, renderTimeText(selectedReport.Thang, selectedReport.Nam), detailData, detailTotal)} 
                                style={styles.btnPrint}
                              >
                                  🖨️ In Báo Cáo
                              </button>
                          </div>
                          {renderTable(detailData, detailTotal)}
                      </div>
                  ) : (
...
const styles = {
  ...
  tabActive: { padding: '10px 20px', border: 'none', background: 'transparent', cursor: 'pointer', color: '#3b82f6', fontWeight: 'bold', fontSize: '15px', borderBottom: '3px solid #3b82f6', marginBottom: '-2px' },
